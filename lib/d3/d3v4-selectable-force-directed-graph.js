...
function createV4SelectableForceDirectedGraph(svg, graph) {
  ...
  Bootstrap.rect = gMain.append("rect")
  ...
  */
  ...
  node
    .append("circle")
    .attr("fill", function (d) { return Bootstrap.colorScheme(d); })
    .attr("r", Bootstrap.defaultRadius)
    .append("title")
    .text(function (d) {
      if ("name" in d)
        return d.name;
  ...
  if (!favSetting) {
    node.append("image")
      .attr("xlink:href", function (d) {
        return "../../images/" + Bootstrap.domainName(d.name) + ".png";
      })
      .on("error", function (d) {
    ...
    node.append("image")
      .attr("xlink:href", function (d) {
        var dName = Bootstrap.domainName(d.name);
        chrome.storage.local.get(dName, function (tNode, result) {
          if (result.hasOwnProperty(dName)) {
            ...
            xhr.open("GET", "https://s2.googleusercontent.com/s2/favicons?domain_url=" + Bootstrap.domainName(d.name), true);
            xhr.send();
          }
        }.bind(null, this));
      })
  ...
  Bootstrap.rect.on("click.a", function () {
  ...
  simulation = d3.forceSimulation(nodes)
    .force("link", d3.forceLink().id(function (d) { return d.index }).distance(100 * Bootstrap.linkDistance))
    .force("collide", d3.forceCollide(function (d) { return 20; }).iterations(1))
    .force("charge", d3.forceManyBody().strength(parentWidth * Bootstrap.nodeCharge))
